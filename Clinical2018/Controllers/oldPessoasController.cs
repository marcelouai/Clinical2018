...
{
    public class oldPessoasController : Controller
    {
        private readonly Clinical2018Context _context;

        public oldPessoasController(Clinical2018Context context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            //deu certo
            //var pessoa = await _context.Pessoa.Include(s => s.Paciente).AsNoTracking().SingleOrDefaultAsync(m => m.PessoaID == id);

            var pessoa = await _context.Pessoa
                .Include(s => s.Paciente)
                .Include(s => s.Paciente.Clinica_Pacientes)
                .Include(s => s.Dentista)
                .Include(s => s.Dentista.Clinica_Dentistas)
