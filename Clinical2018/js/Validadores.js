function MascaraNumero(campo,tipo) {
    var chrDigito = String.fromCharCode(event.keyCode);
    //Tipo = Tipo.toLowerCase();
    //alert('erro ' + campo.value);
    ...
    switch (tipo)
    {
        case "telefone":
            //alert('telefone ' + tipo);
            if (campo.value.length == 1) { campo.value = '(' + campo.value; }
...
//    var chrDigito = campo;//String.fromCharCode(event.keyCode);
...
//function mascaraInteiro(){	if (event.keyCode < 48 || event.keyCode > 57){		event.returnValue = false;		return false;	}	return true;}
...
//function MascaraCep(cep) { if (mascaraInteiro(cep) == false) { event.returnValue = false; } return formataCampo(cep, '00.000-000', event); }
...
//function formataCampo(campo, Mascara, evento) { 	var boleanoMascara; 		var Digitato = evento.keyCode;	exp = /\-|\.|\/|\(|\)| /g	campoSoNumeros = campo.value.toString().replace( exp, "" );    	var posicaoCampo = 0;	 	var NovoValorCampo="";	var TamanhoMascara = campoSoNumeros.length;; 		if (Digitato != 8) 
//{     
//    // backspace 		
